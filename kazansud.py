...
def wright_to_file(result):
    ...
    writer = pd.ExcelWriter('kazan_sud_result.xlsx')
    ...
    async with (aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session):
        for cur_state in cur_states:
            ...
            for item in codex:
                ...
                params["codex"] = item
                async with session.get(kazan_sud_url, params=params, headers=headers, cookies=cookies) as response:
                    await asyncio.sleep(5)
                    soup = bs(await response.text(), 'lxml')
                    paginator = soup.find("input", id="paginationFormMaxPages")
                    if not paginator:
                        params['page'] = "1"
                        tasks.append(params.copy())
                        continue
                    paginator = paginator.get('value')
        ...
        for param in alive_it(tasks):
            async with session.get(kazan_sud_url, params=param, headers=headers) as page_response:
                await asyncio.sleep(2)
                try:
                    soup = bs(await page_response.text(), 'lxml')
                    data_rows = soup.find("table", class_="list-table").find_all('tr')[1:]
                    state = True if param["publishingState"] == "80c39ae" else False
                    for row in data_rows:
                        row = row.find_all('td')
                        side = row[1].find("span").text.strip()

                        row_result = {"СТ": param["codex"],
                                      "ДЕЛО": row[0].text.strip(),
                                      "УЧ": row[3].find("a").text.strip(),
